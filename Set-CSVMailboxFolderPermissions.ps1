<#
    # Set-CSVMailboxFolderPermissions
    .SYNOPSIS
    Set mailbox folder permissions in bulk from a CSV file.

    .DESCRIPTION
    Uses Set-MailboxFolderPermission to change existing permissions for the specified user.  Will try using the FolderPath first,
    and if unsuccessful, typically due to special characters in the folder path, FolderId will be attempted instead.  If still
    unable, an error message will be written (Write-Host (red), not a thrown error).
    ...
    Version History:
        - 2025-10-14: Created initial script.
        - 2025-TBD..: v1.0.0
#>
[CmdletBinding()]
param(
...
if (-not (Get-RequiredCommands Add-MailboxFolderPermission, Set-MailboxFolderPermission)) {
    throw 'This script requires an active session to Exchange/EXO and access to the Set-MailboxFolderPermission cmdlets.'
}
